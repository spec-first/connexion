...
class FormDataValidator(AbstractRequestBodyValidator):
    ...
    async def _parse(self, stream: t.AsyncGenerator[bytes, None], scope: Scope) -> dict:
        ...
        if self._uri_parser is not None:
            ...
            form_data = {}
            file_data: t.Dict[str, t.Union[str, t.List[str], UploadFile, t.List[UploadFile]]] = {}
            upload_files = {}
            ...
            for key in data.keys():
                ...
                if value and isinstance(value[0], UploadFile):
                    ...
                    is_openapi31_allof = (hasattr(self._schema, "get") and 
                                         self._schema.get("components", {}) is not None and 
                                         "allOf" in self._schema)
            ...
            for key in data.keys():
                ...
                if "properties" in self._schema and key in self._schema.get("properties", {}):
                    ...
                    for schema in self._schema["allOf"]:
                        if "allOf" in schema:
                            for sub_schema in schema["allOf"]:
                                if "properties" in sub_schema and key in sub_schema["properties"]:
                ...
                def is_file(schema):
                    ...
                    if schema.get("type") == "string" and schema.get("format") in ["binary", "base64"]:
                        return True
                    ...
                    if "allOf" in schema:
                        for sub_schema in schema["allOf"]:
                            if isinstance(sub_schema, dict):
                                if sub_schema.get("type") == "string" and sub_schema.get("format") in ["binary", "base64"]:
                                    return True
                ...
                is_array_of_files = False if not param_schema else is_file(param_schema.get("items", {}))
                ...
                if param_schema and param_schema.get('type') == 'string' and len(value) == 1:
        ...
        for key, value in body.items():
            ...
            if isinstance(value, UploadFile):
                ...
                if not is_array_schema and "allOf" in self._schema:
                    for schema in self._schema.get("allOf", []):
                        if "properties" in schema and key in schema.get("properties", {}):
                            is_array_schema = schema.get("properties", {}).get(key, {}).get("type") == "array"
                            if is_array_schema:
                                break
