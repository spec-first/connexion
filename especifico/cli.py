...
starting point for developing your API with Especifico.
"""
...
import especifico
from especifico.mock import MockResolver

logger = logging.getLogger("especifico.cli")
...
AVAILABLE_APPS = {
    FLASK_APP: "especifico.apps.flask_app.FlaskApp",
    AIOHTTP_APP: "especifico.apps.aiohttp_app.AioHttpApp",
}
...
def print_version(ctx, param, value):
    if not value or ctx.resilient_parsing:
        return
    click.echo(f"Especifico {especifico.__version__}")
    ctx.exit()
...
@click.option(
    "--wsgi-server",
    "-w",
    type=click.Choice(sorted(AVAILABLE_SERVERS.keys())),
...
def run(
    ...
    app_cls = especifico.utils.get_function_from_name(AVAILABLE_APPS[app_framework])
