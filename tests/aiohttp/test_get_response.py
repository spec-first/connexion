...
from especifico.apis.aiohttp_api import AioHttpApi
...
async def test_get_response_from_aiohttp_response(api):
    response = await api.get_response(
        web.Response(text="foo", status=201, headers={"X-header": "value"}),
    )
...
async def test_get_response_from_aiohttp_stream_response(api):
    response = await api.get_response(
        web.StreamResponse(status=201, headers={"X-header": "value"}),
    )
...
async def test_get_response_from_especifico_response(api):
    response = await api.get_response(
        EspecificoResponse(
            status_code=201,
            mimetype="text/plain",
...
async def test_get_response_from_tuple_error(api):
    with pytest.raises(TypeError) as e:
        await api.get_response(
            (web.Response(text="foo", status=201, headers={"X-header": "value"}), 200),
        )
    assert (
        str(e.value)
...
async def test_get_especifico_response_from_aiohttp_response(api):
    response = api.get_especifico_response(
        web.Response(text="foo", status=201, headers={"X-header": "value"}),
    )
    assert isinstance(response, EspecificoResponse)
...
async def test_get_especifico_response_from_especifico_response(api):
    response = api.get_especifico_response(
        EspecificoResponse(
            status_code=201,
...
async def test_get_especifico_response_from_tuple(api):
    response = api.get_especifico_response(("foo", 201, {"X-header": "value"}))
    assert isinstance(response, EspecificoResponse)
    assert response.status_code == 201
    assert response.body == b"foo"
    assert response.content_type == "text/plain"
...
async def test_get_especifico_response_from_aiohttp_stream_response(api):
    response = api.get_especifico_response(
        web.StreamResponse(status=201, headers={"X-header": "value"}),
    )
    assert isinstance(response, EspecificoResponse)
