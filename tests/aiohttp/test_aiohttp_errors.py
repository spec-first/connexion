...
from especifico.apis.aiohttp_api import HTTPStatus
...
def aiohttp_app(problem_api_spec_dir):
    app = AioHttpApp(__name__, port=5001, specification_dir=problem_api_spec_dir, debug=True)
...
async def test_aiohttp_problems_misc(aiohttp_app, aiohttp_client):
    ...
    custom_problem = await app_client.get(
        "/v1.0/customized_problem_response",
    )  # type: aiohttp.ClientResponse
    ...
    problem_as_exception = await app_client.get(
        "/v1.0/problem_exception_with_extra_args",
    )  # type: aiohttp.ClientResponse
...
@pytest.mark.skip(
    reason="aiohttp_api.get_especifico_response uses _cast_body "
