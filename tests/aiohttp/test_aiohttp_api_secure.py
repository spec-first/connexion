...
def oauth_aiohttp_client(monkeypatch):
    async def fake_get(url, params=None, headers=None, timeout=None):
        ...
        if token in ["100", "has_myscope"]:
            return FakeAioHttpClientResponse(
    ...
    monkeypatch.setattr(
        "aiohttp.ClientSession", MagicMock(return_value=client_instance)
    )


async def test_auth_all_paths(
    oauth_aiohttp_client, aiohttp_api_spec_dir, aiohttp_client
):
    app = AioHttpApp(
        __name__,
        port=5001,
    ...
    get_inexistent_endpoint = await app_client.get(
        "/v1.0/does-not-exist-valid-token", headers={"Authorization": "Bearer 100"}
    )
    assert get_inexistent_endpoint.status == 404
    assert get_inexistent_endpoint.content_type == "application/problem+json"

    get_inexistent_endpoint = await app_client.get("/v1.0/does-not-exist-no-token")
    assert get_inexistent_endpoint.status == 401
    assert get_inexistent_endpoint.content_type == "application/problem+json"


@pytest.mark.parametrize("spec", ["swagger_secure.yaml", "openapi_secure.yaml"])
async def test_secure_app(
    oauth_aiohttp_client, aiohttp_api_spec_dir, aiohttp_client, spec
):
    """
    ...
    response = await app_client.get(
        "/v1.0/all_auth", headers={"Authorization": "Bearer 100"}
    )
    ...
    assert (
        response.status == 200
    ), "Authorization header in lower case should be accepted"
    assert (await response.json()) == {"scope": ["myscope"], "uid": "test-user"}
    ...
    assert (
        response.status == 200
    ), "Authorization header in upper case should be accepted"
    assert (await response.json()) == {"scope": ["myscope"], "uid": "test-user"}

    basic_header = "Basic " + base64.b64encode(b"username:username").decode("ascii")
    response = await app_client.get(
        "/v1.0/all_auth", headers={"Authorization": basic_header}
    )
    assert response.status == 200
    ...
    response = await app_client.get(
        "/v1.0/all_auth", headers={"X-API-Key": '{"foo": "bar"}'}
    )
    assert response.status == 200
...
async def test_bearer_secure(aiohttp_api_spec_dir, aiohttp_client):
    ...
    response = await app_client.get(
        "/v1.0/bearer_auth", headers={"Authorization": bearer_header}
    )
    assert response.status == 200
    assert (await response.json()) == {"scope": ["myscope"], "uid": "test-user"}
...
async def test_async_secure(aiohttp_api_spec_dir, aiohttp_client):
    ...
    bearer_header = 'Bearer {"scope": ["myscope"], "uid": "test-user"}'
    response = await app_client.get(
        "/v1.0/async_auth", headers={"Authorization": bearer_header}
    )
    assert response.status == 200
    assert (await response.json()) == {"scope": ["myscope"], "uid": "test-user"}
    ...
    assert (
        response.status == 403
    ), "async_scope_validation should deny access if scopes are not strictly the same"

    basic_header = "Basic " + base64.b64encode(b"username:username").decode("ascii")
    response = await app_client.get(
        "/v1.0/async_auth", headers={"Authorization": basic_header}
    )
    assert response.status == 200
    assert (await response.json()) == {"uid": "username"}
    ...
    bearer_header = 'Bearer {"scope": ["myscope"], "uid": "test-user"}'
    response = await app_client.get(
        "/v1.0/async_bearer_auth", headers={"Authorization": bearer_header}
    )
    assert response.status == 200
    assert (await response.json()) == {"scope": ["myscope"], "uid": "test-user"}
...
@pytest.mark.parametrize("spec", ["swagger_secure.yaml", "openapi_secure.yaml"])
async def test_auth_exception_swagger(
    oauth_aiohttp_client, aiohttp_api_spec_dir, aiohttp_client, spec
):
    app = AioHttpApp(
        __name__, port=5001, specification_dir=aiohttp_api_spec_dir, debug=True
    ...
    response = await app_client.get("/v1.0/fail_auth", headers={"X-API-Key-2": "foo"})
    assert response.status == 401
    assert response.content_type == "application/problem+json"
