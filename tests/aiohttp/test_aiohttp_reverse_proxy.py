...
class XPathForwarded(XForwardedBase):
    ...
    async def test_swagger_json_behind_proxy(simple_api_spec_dir, aiohttp_client):
        ...
        app = AioHttpApp(__name__, port=5001, specification_dir=simple_api_spec_dir, debug=True)
        ...
        assert b'url = "/behind/proxy/v1.0/swagger.json"' in (await swagger_ui.read())
        ...
        assert (
            api.specification.raw["basePath"] == "/v1.0"
        ), "Original specifications should not have been changed"

        assert (
            json_.get("basePath") == "/behind/proxy/v1.0"
    ...
    async def test_openapi_json_behind_proxy(simple_api_spec_dir, aiohttp_client):
        ...
        swagger_ui = await app_client.get("/v1.0/ui/", headers=headers)
        assert swagger_ui.status == 200
        assert b'url: "/behind/proxy/v1.0/openapi.json"' in (await swagger_ui.read())
        ...
        assert (
            json_.get("servers", [{}])[0].get("url") == "/behind/proxy/v1.0"
        ), "basePath should contains original URI"

        url = api.specification.raw.get("servers", [{}])[0].get("url")
        assert url != "/behind/proxy/v1.0", "Original specifications should not have been changed"

        json_["servers"] = api.specification.raw.get("servers")
        assert api.specification.raw == json_, "Only there servers block should have been updated"
