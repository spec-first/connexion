...
import aiohttp.web
import pytest
from aiohttp.test_utils import aiohttp_client
...
@asyncio.coroutine
def test_response_with_str_body(aiohttp_app):
    # Create the app and run the test_app testcase below.
    app_client = yield from aiohttp_client(aiohttp_app.app)
...
@asyncio.coroutine
def test_response_with_non_str_and_non_json_body(aiohttp_app):
    app_client = yield from aiohttp_client(aiohttp_app.app)
...
@asyncio.coroutine
def test_response_with_bytes_body(aiohttp_app):
    # Create the app and run the test_app testcase below.
    app_client = yield from aiohttp_client(aiohttp_app.app)
...
@asyncio.coroutine
def test_validate_responses(aiohttp_app):
    app_client = yield from aiohttp_client(aiohttp_app.app)
...
@asyncio.coroutine
def test_get_users(aiohttp_app):
    app_client = yield from aiohttp_client(aiohttp_app.app)
...
@asyncio.coroutine
def test_access_request_context(aiohttp_app):
    app_client = yield from aiohttp_client(aiohttp_app.app)
...
@asyncio.coroutine
def test_query_parsing_simple(aiohttp_app):
...
@asyncio.coroutine
def test_query_parsing_array(aiohttp_app):
...
@asyncio.coroutine
def test_query_parsing_array_multi(aiohttp_app):
...
if sys.version_info[0:2] >= (3, 5):
    ...
    @asyncio.coroutine
    def test_validate_responses_async_def(aiohttp_app_async_def):
