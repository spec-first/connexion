...
def upload_with_ref(body, **kwargs):
    ...
    if isinstance(file_name, list) and len(file_name) == 1:
        ...
        try:
            ...
            print(
                f"Converting fileName from string representation of list: {file_name}",
                file=sys.stderr,
            )
        except:
            pass
    ...
    if isinstance(description, list) and len(description) == 1:
        ...
        print(
            f"Converting description from list to string: {description}",
            file=sys.stderr,
        )
    elif (
        ...
        try:
            ...
            print(
                f"Converting description from string representation of list: {description}",
                file=sys.stderr,
            )
        except:
            pass
    ...
    if not file or not file_name:
        print(
            f"MISSING DATA: file={file is not None}, fileName={file_name is not None}",
            file=sys.stderr,
        )
