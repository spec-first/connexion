...
import especifico
from especifico.cli import main


@pytest.fixture()
def mock_app_run(mock_get_function_from_name):
    test_server = MagicMock(wraps=especifico.FlaskApp(__name__))
...
def mock_get_function_from_name(monkeypatch):
    ...
    monkeypatch.setattr(
        "especifico.cli.especifico.utils.get_function_from_name", get_function_from_name,
    )
...
def expected_arguments():
    """
    Default values arguments used to call `especifico.App` by cli.
    """
    return {
        "options": {
            "serve_spec": True,
...
def test_print_version():
    ...
    assert f"Especifico {especifico.__version__}" in result.output
...
def test_run_simple_spec(mock_app_run, spec_file):
    ...
    app_instance.run.assert_called_with(port=default_port, host=None, server="flask", debug=False)
...
def test_run_no_options_all_default(mock_app_run, expected_arguments, spec_file):
    runner = CliRunner()
    runner.invoke(main, ["run", spec_file], catch_exceptions=False)
    mock_app_run.assert_called_with("especifico.cli", **expected_arguments)
...
def test_run_using_option_console_ui_from(mock_app_run, expected_arguments, spec_file):
    ...
    runner = CliRunner()
    runner.invoke(main, ["run", spec_file, "--console-ui-from", user_path], catch_exceptions=False)
...
def test_run_using_option_console_ui_url(mock_app_run, expected_arguments, spec_file):
    ...
    runner = CliRunner()
    runner.invoke(main, ["run", spec_file, "--console-ui-url", user_url], catch_exceptions=False)
...
def test_run_in_debug_mode(mock_app_run, expected_arguments, spec_file, monkeypatch):
    logging_config = MagicMock(name="especifico.cli.logging.basicConfig")
    monkeypatch.setattr("especifico.cli.logging.basicConfig", logging_config)
...
def test_run_in_very_verbose_mode(mock_app_run, expected_arguments, spec_file, monkeypatch):
    logging_config = MagicMock(name="especifico.cli.logging.basicConfig")
    monkeypatch.setattr("especifico.cli.logging.basicConfig", logging_config)
...
def test_run_using_option_base_path(mock_app_run, expected_arguments, spec_file):
    runner = CliRunner()
    runner.invoke(main, ["run", spec_file, "--base-path", "/foo"], catch_exceptions=False)
...
def test_run_with_wsgi_containers(mock_app_run, spec_file):
    ...
    # missing gevent
    result = runner.invoke(main, ["run", spec_file, "-w", "gevent"], catch_exceptions=False)
    assert "gevent library is not installed" in result.output
    assert result.exit_code == 1

    # missing tornado
    result = runner.invoke(main, ["run", spec_file, "-w", "tornado"], catch_exceptions=False)
    assert "tornado library is not installed" in result.output
    assert result.exit_code == 1

    # using flask
    result = runner.invoke(main, ["run", spec_file, "-w", "flask"], catch_exceptions=False)
    assert result.exit_code == 0
...
def test_run_with_aiohttp_not_installed(mock_app_run, spec_file):
    ...
    # missing aiohttp
    result = runner.invoke(main, ["run", spec_file, "-f", "aiohttp"], catch_exceptions=False)
...
def test_run_with_incompatible_server_and_default_framework(mock_app_run, spec_file):
    ...
    result = runner.invoke(main, ["run", spec_file, "-s", "aiohttp"], catch_exceptions=False)
...
def test_run_with_incompatible_server_and_framework(mock_app_run, spec_file):
    ...
    result = runner.invoke(
        main, ["run", spec_file, "-s", "flask", "-f", "aiohttp"], catch_exceptions=False,
    )
