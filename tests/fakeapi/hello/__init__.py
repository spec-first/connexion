...
from especifico import context, NoContent, ProblemException, request
...
def post_greeting3(body, **kwargs):
    data = {"greeting": f'Hello {body["name"]}'}
    return data
...
def get_bye_secure_from_flask():
    return f'Goodbye {context["user"]} (Secure!)'


def get_bye_secure_from_especifico(req_context):
    return f'Goodbye {req_context["user"]} (Secure!)'
...
def get_custom_problem_response():
    raise ProblemException(403, "You need to pay", "Missing amount", ext={"amount": 23.0})


def throw_problem_exception():
    raise ProblemException(title="As Exception", detail="Something wrong or not!", ext={"age": 30})
