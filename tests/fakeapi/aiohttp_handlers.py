...
async def get_bye(name):
    return aiohttp.web.Response(text=f"Goodbye {name}")
...
async def get_datetime():
    return EspecificoResponse(body={"value": datetime.datetime(2000, 1, 2, 3, 4, 5, 6)})


async def get_date():
    return EspecificoResponse(body={"value": datetime.date(2000, 1, 2)})
