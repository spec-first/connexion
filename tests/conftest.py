...
def oauth_requests(monkeypatch):
    ...
    monkeypatch.setattr("especifico.security.flask_security_handler_factory.session.get", fake_get)
...
def schema_app(request):
    return build_app_from_fixture("different_schemas", request.param, validate_responses=True)
