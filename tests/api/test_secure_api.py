...
def test_security(oauth_requests, secure_endpoint_app):
    ...
    assert (
        get_bye_wrong_scope_reponse["detail"] == "Provided token doesn't have the required scope"
    )
    ...
    headers = {"Authorization": "Bearer 100"}
    get_bye_from_especifico = app_client.get(
        "/v1.0/byesecure-from-especifico", headers=headers,
    )  # type: flask.Response
    assert get_bye_from_especifico.data == b"Goodbye test-user (Secure!)"

    headers = {"Authorization": "Bearer 100"}
    get_bye_from_especifico = app_client.get(
        "/v1.0/byesecure-jwt/test-user", headers=headers,
    )  # type: flask.Response
    assert get_bye_from_especifico.data == b"Goodbye test-user (Secure: 100)"
