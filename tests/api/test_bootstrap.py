...
from conftest import build_app_from_fixture, TEST_FOLDER
import flask
import jinja2
import pytest
import yaml
...
from especifico.exceptions import InvalidSpecification
from especifico.http_facts import METHODS
from especifico.json_schema import ExtendedSafeLoader
...
def test_app_with_resolver(simple_api_spec_dir, spec):
    from especifico.resolver import Resolver
...
def test_app_with_different_uri_parser(simple_api_spec_dir):
    from especifico.decorators.uri_parsing import FirstValueURIParser
...
def test_swagger_ui(simple_api_spec_dir, spec):
    ...
    spec_json_filename = f'/v1.0/{spec.replace("yaml", "json")}'
...
def test_no_swagger_ui_config_json(simple_api_spec_dir, spec):
    """
    Verify the swagger-ui-config.json file is not returned when the swagger_ui_config option not
    passed to app.
    """
    app = App(__name__, port=5001, specification_dir=simple_api_spec_dir, debug=True)
    app.add_api(spec)
...
def test_swagger_json_app(simple_api_spec_dir, spec):
    ...
    app.add_api(spec)
    app_client = app.app.test_client()
    url = f'/v1.0/{spec.replace("yaml", "json")}'
