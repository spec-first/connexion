...
from especifico.apps.flask_app import FlaskJSONEncoder
...
def test_openapi_yaml_behind_proxy(reverse_proxied_app):
    ...
    openapi_yaml = app_client.get("/v1.0/" + reverse_proxied_app._spec_file, headers=headers)
...
def test_jsonifier(simple_app):
    ...
    assert get_greetings.content_type == "application/x.especifico+json"
