...
def test_validator_map(json_validation_spec_dir, spec):
    ...
    res = app_client.post('/v1.0/minlength', data=json.dumps({'foo': ''}),
...
def test_readonly(json_validation_spec_dir, spec):
    ...
    # type: flask.Response
    res = app_client.post(
        '/v1.0/user', data=json.dumps({'name': 'max', 'password': '1234'}), content_type='application/json')
    assert res.status_code == 200
    assert json.loads(res.data.decode()).get('user_id') == 8

    # type: flask.Response
    res = app_client.post('/v1.0/user', data=json.dumps({'user_id': 9, 'name': 'max'}), content_type='application/json')
    assert res.status_code == 400
